...
function sma(data,n){return data.map(function(_,i){if(i<n-1)return null;return data.slice(i-n+1,i+1).reduce(function(s,d){return s+d.c;},0)/n;});}
function bb(data){var m=sma(data,20);return data.map(function(_,i){if(i<19)return{m:null,u:null,l:null};var sl=data.slice(i-19,i+1),mn=m[i],std=Math.sqrt(sl.reduce(function(s,d){return s+(d.c-mn)*(d.c-mn);},0)/20);return{m:mn,u:mn+2*std,l:mn-2*std};});}
function rsi(data){return data.map(function(_,i){if(i<14)return null;var g=0,l=0;for(var j=i-13;j<=i;j++){var d=data[j].c-data[j-1].c;if(d>0)g+=d;else l-=d;}return 100-100/(1+g/(l||1e-9));});}
function signals(data,s5,s20,rv,bv){var out=[];for(var i=20;i<data.length;i++){var d=data[i];if(s5[i-1]<s20[i-1]&&s5[i]>=s20[i])out.push({i:i,t:"buy",r:"골든크로스",score:3});if(s5[i-1]>s20[i-1]&&s5[i]<=s20[i])out.push({i:i,t:"sell",r:"데드크로스",score:3});if(rv[i]<30&&rv[i-1]>=30)out.push({i:i,t:"buy",r:"RSI 과매도",score:2});if(rv[i]>70&&rv[i-1]<=70)out.push({i:i,t:"sell",r:"RSI 과매수",score:2});if(bv[i].l&&d.l<bv[i].l&&d.c>bv[i].l)out.push({i:i,t:"buy",r:"볼린저 하단",score:1});if(bv[i].u&&d.h>bv[i].u&&d.c<bv[i].u)out.push({i:i,t:"sell",r:"볼린저 상단",score:1});}return out;}
function backtest(data,sigs){var cash=10000000,shares=0,trades=[];sigs.forEach(function(sig){var d=data[sig.i];if(!d)return;if(sig.t==="buy"&&cash>d.c){var q=Math.floor(cash*0.5/d.c);if(q>0){shares+=q;cash-=q*d.c;trades.push(Object.assign({},sig,{price:d.c,qty:q,date:d.date,action:"매수"}));}}else if(sig.t==="sell"&&shares>0){var q2=Math.floor(shares*0.5);if(q2>0){cash+=q2*d.c;shares-=q2;trades.push(Object.assign({},sig,{price:d.c,qty:q2,date:d.date,action:"매도"}));}}});var last=data[data.length-1],first=data[0];var fin=cash+shares*(last?last.c:0);return{trades:trades,roi:(fin-10000000)/10000000*100,bh:last&&first?(last.c/first.c-1)*100:0,fin:fin};}

var STOCKS={KR:[{sym:"005930",name:"삼성전자",sector:"반도체",base:73000},{sym:"000660",name:"SK하이닉스",sector:"반도체",base:195000},{sym:"035420",name:"NAVER",sector:"IT",base:218000},{sym:"005380",name:"현대차",sector:"자동차",base:210000},{sym:"035720",name:"카카오",sector:"IT",base:42000}],US:[{sym:"AAPL",name:"Apple",sector:"Tech",base:198},{sym:"NVDA",name:"NVIDIA",sector:"Semicon",base:875},{sym:"TSLA",name:"Tesla",sector:"EV",base:245},{sym:"MSFT",name:"Microsoft",sector:"Tech",base:415},{sym:"META",name:"Meta",sector:"Social",base:520}]};
var cache={};
function getData(s){if(!cache[s.sym])cache[s.sym]=genOHLC(s.base);return cache[s.sym];}
...
function Chart(props){
  ...
  useEffect(function(){
    if(!data.length||!mRef.current)return;
    ...
    var W=cv.offsetWidth,H=300,pd={t:16,r:62,b:28,l:8},cw=W-pd.l-pd.r,ch=H-pd.t-pd.b;
    var vis=data.slice(-60),vi=data.length-60,bvv=bv.slice(-60);
    var allP=vis.reduce(function(a,d){return a.concat([d.h,d.l]);},[]); 
    bvv.forEach(function(b){if(b.u)allP.push(b.u);if(b.l)allP.push(b.l);});
    ...
    ["u","l","m"].forEach(function(k,ki){ctx.strokeStyle="rgba(100,180,255,0.3)";ctx.lineWidth=1;ctx.setLineDash(ki===2?[3,3]:[]);ctx.beginPath();var st=false;bvv.forEach(function(b,i){if(!b[k])return;var x=pd.l+i*bw+bw/2;if(!st){ctx.moveTo(x,toY(b[k]));st=true;}else ctx.lineTo(x,toY(b[k]));});ctx.stroke();});ctx.setLineDash([]);
    [[s5.slice(-60),"#FFD700"],[s20.slice(-60),"#FF6B6B"]].forEach(function(pr){ctx.strokeStyle=pr[1];ctx.lineWidth=1.5;ctx.beginPath();var st=false;pr[0].forEach(function(v,i){if(!v)return;var x=pd.l+i*bw+bw/2;if(!st){ctx.moveTo(x,toY(v));st=true;}else ctx.lineTo(x,toY(v));});ctx.stroke();});
    ...
    ctx.fillStyle="rgba(255,255,255,0.2)";ctx.font="9px monospace";ctx.textAlign="center";[0,15,30,45,59].forEach(function(i){if(!vis[i])return;ctx.fillText((vis[i].date.getMonth()+1)+"/"+vis[i].date.getDate(),pd.l+i*bw+bw/2,H-8);});
  },[data,s5,s20,bv,sigs]);
  useEffect(function(){
    if(!ind||ind==="none"||!sRef.current||!data.length)return;
    var cv=sRef.current,dpr=window.devicePixelRatio||1;
    cv.width=cv.offsetWidth*dpr;cv.height=80*dpr;cv.style.height="80px";
    var ctx=cv.getContext("2d");ctx.scale(dpr,dpr);
    var W=cv.offsetWidth,H=80,pd={t:8,r:62,b:18,l:8},cw=W-pd.l-pd.r,ch=H-pd.t-pd.b,bw=cw/60;
    ctx.clearRect(0,0,W,H);
    if(ind==="rsi"){
      var rv2=rsi(data).slice(-60);
      ...
      ctx.strokeStyle="#A78BFA";ctx.lineWidth=1.5;ctx.beginPath();var st=false;rv2.forEach(function(v,i){if(!v)return;var x=pd.l+i*bw+bw/2,y=pd.t+ch-(v/100)*ch;if(!st){ctx.moveTo(x,y);st=true;}else ctx.lineTo(x,y);});ctx.stroke();
...
export default function Home(){
  ...
  var ss=useState(STOCKS.KR[0]),stock=ss[0],setStock=ss[1];
  var ts=useState("chart"),tab=ts[0],setTab=ts[1];
  var is=useState("rsi"),ind=is[0],setInd=is[1];
  var bs=useState(null),btR=bs[0],setBtR=bs[1];
  var as=useState(""),aiText=as[0],setAiText=as[1];
  var als=useState(false),aiLoad=als[0],setAiLoad=als[1];

  var data=getData(stock),s5=sma(data,5),s20=sma(data,20),bv=bb(data),rv=rsi(data);
  var sigs=signals(data,s5,s20,rv,bv);
  ...
  var isKR=market==="KR";
  var fmt=function(v){return isKR?v.toLocaleString()+"₩":"$"+v.toFixed(2);};
  ...
  var doAI=async function(){
    ...
    try{
      var res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:stock.name,sym:stock.sym,market:market,price:fmt(last.c),chg:chg.toFixed(2),rsi:lastRSI?lastRSI.toFixed(1):"—",signal:lastSig?(lastSig.t==="buy"?"🟢매수 ":"🔴매도 ")+lastSig.r:"없음",recent5:data.slice(-5).map(function(d){return fmt(d.c);}).join(", "),s5:s5[s5.length-1]?s5[s5.length-1].toFixed(0):"—",s20:s20[s20.length-1]?s20[s20.length-1].toFixed(0):"—",bbU:bv[bv.length-1]&&bv[bv.length-1].u?bv[bv.length-1].u.toFixed(0):"—",bbL:bv[bv.length-1]&&bv[bv.length-1].l?bv[bv.length-1].l.toFixed(0):"—"})});
      var json=await res.json();setAiText(json.text||"분석 실패");
  ...
  return React.createElement("div",{style:{minHeight:"100vh",background:"linear-gradient(160deg,#080d18,#0f1a2e 60%,#080d18)",color:"#fff",fontFamily:"monospace",fontSize:13}},
    React.createElement("style",null,"*{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:2px}"),
    React.createElement("div",{style:{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",borderBottom:"1px solid rgba(255,255,255,0.08)",flexWrap:"wrap"}},
      ...
      React.createElement("div",{style:{marginLeft:"auto",display:"flex",gap:6}},
        ["KR","US"].map(function(m){return React.createElement("button",{key:m,onClick:function(){setMarket(m);setStock(STOCKS[m][0]);setAiText("");setBtR(null);},style:{padding:"5px 14px",borderRadius:6,border:"1px solid "+(market===m?C.g:C.b),background:market===m?"rgba(0,229,160,0.08)":"transparent",color:market===m?C.g:C.m,cursor:"pointer",fontFamily:"inherit",fontSize:11}},m==="KR"?"🇰🇷 국내":"🇺🇸 미국");})
      )
    ),
    React.createElement("div",{style:{display:"flex",height:"calc(100vh - 49px)"}},
      React.createElement("div",{style:{width:168,borderRight:"1px solid rgba(255,255,255,0.08)",overflowY:"auto",padding:10,flexShrink:0}},
        ...
        STOCKS[market].map(function(s){
          var sd=getData(s),sl=sd[sd.length-1]?sd[sd.length-1].c:s.base,sp=sd[sd.length-2]?sd[sd.length-2].c:s.base,sc=(sl-sp)/sp*100,sel=stock.sym===s.sym;
          return React.createElement("div",{key:s.sym,onClick:function(){setStock(s);setAiText("");setBtR(null);},style:{padding:"9px 8px",borderRadius:8,cursor:"pointer",marginBottom:3,background:sel?"rgba(0,229,160,0.07)":"transparent",border:"1px solid "+(sel?"rgba(0,229,160,0.25)":"transparent")}},
            ...
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",marginTop:5,fontSize:11}},
              React.createElement("span",null,isKR?sl.toLocaleString():"$"+sl.toFixed(1)),
              React.createElement("span",{style:{color:sc>=0?C.g:C.r}},(sc>=0?"▲":"▼")+Math.abs(sc).toFixed(1)+"%")
            )
          );
        })
      ),
      React.createElement("div",{style:{flex:1,overflowY:"auto",padding:12,display:"flex",flexDirection:"column",gap:10}},
        React.createElement("div",{style:Object.assign({},card,{display:"flex",alignItems:"center",gap:20,flexWrap:"wrap"})},
          React.createElement("div",null,React.createElement("div",{style:{fontSize:16,fontWeight:600}},stock.name),React.createElement("div",{style:{fontSize:10,color:C.m,marginTop:2}},stock.sym+" · "+stock.sector)),
          ...
          React.createElement("div",{style:{display:"flex",gap:16,flexWrap:"wrap"}},
            [["RSI",lastRSI?lastRSI.toFixed(1):"—",lastRSI<30?C.g:lastRSI>70?C.r:"#fff"],["신호",lastSig?(lastSig.t==="buy"?"🟢매수":"🔴매도"):"—",C.p],["신호수",sigs.length+"개",C.gold]].map(function(it){
          ...
          React.createElement("div",{style:{marginLeft:"auto",display:"flex",gap:6,flexWrap:"wrap"}},
            [["chart","📊 차트"],["backtest","🔄 백테스트"],["portfolio","💼 포트폴리오"],["ai","🤖 AI분석"]].map(function(it){
              return React.createElement("button",{key:it[0],onClick:function(){setTab(it[0]);},style:{padding:"6px 12px",borderRadius:7,border:"1px solid "+(tab===it[0]?C.p:C.b),background:tab===it[0]?"rgba(167,139,250,0.1)":"transparent",color:tab===it[0]?C.p:C.m,cursor:"pointer",fontFamily:"inherit",fontSize:11}},it[1]);
            })
          )
        ),
        tab==="chart"&&React.createElement(React.Fragment,null,
          React.createElement("div",{style:Object.assign({},card,{padding:0,overflow:"hidden"})},
            React.createElement("div",{style:{display:"flex",gap:6,padding:"8px 12px",borderBottom:"1px solid rgba(255,255,255,0.08)",alignItems:"center",flexWrap:"wrap"}},
              ...
              [["rsi","RSI"],["macd","MACD"],["none","없음"]].map(function(it){return React.createElement("button",{key:it[0],onClick:function(){setInd(it[0]);},style:{padding:"3px 10px",borderRadius:5,border:"1px solid "+(ind===it[0]?C.gold:C.b),background:ind===it[0]?"rgba(255,215,0,0.08)":"transparent",color:ind===it[0]?C.gold:C.m,cursor:"pointer",fontFamily:"inherit",fontSize:11}},it[1]);}),
              React.createElement("div",{style:{marginLeft:"auto",display:"flex",gap:10,fontSize:10,color:C.m,flexWrap:"wrap"}},
                [["#FFD700","MA5"],["#FF6B6B","MA20"],["rgba(100,180,255,0.7)","볼린저"],[C.g,"매수▲"],[C.r,"매도▼"]].map(function(it){return React.createElement("span",{key:it[1],style:{display:"flex",alignItems:"center",gap:4}},React.createElement("span",{style:{width:7,height:7,borderRadius:"50%",background:it[0],display:"inline-block"}}),it[1]);})
              )
            ),
            React.createElement(Chart,{data:data,s5:s5,s20:s20,bv:bv,sigs:sigs,ind:ind})
          ),
          React.createElement("div",{style:card},
            ...
            React.createElement("div",{style:{overflowX:"auto"}},
              React.createElement("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:12}},
                ...
                React.createElement("tbody",null,sigs.slice(-8).reverse().map(function(sig,i){var d=data[sig.i];return React.createElement("tr",{key:i},
                  React.createElement("td",{style:{padding:"6px 8px",color:C.m}},d?d.date.toLocaleDateString("ko-KR"):"—"),
                  React.createElement("td",{style:{padding:"6px 8px"}},React.createElement("span",{style:{padding:"2px 9px",borderRadius:20,fontSize:11,background:sig.t==="buy"?"rgba(0,229,160,0.12)":"rgba(255,77,109,0.12)",color:sig.t==="buy"?C.g:C.r}},sig.t==="buy"?"매수":"매도")),
                  React.createElement("td",{style:{padding:"6px 8px"}},sig.r),
                  React.createElement("td",{style:{padding:"6px 8px"}},"⭐".repeat(sig.score)),
                  React.createElement("td",{style:{padding:"6px 8px"}},d?fmt(d.c):"—")
                );}))
              )
            )
          )
        ...
        tab==="backtest"&&React.createElement("div",{style:card},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}},
            ...
            React.createElement("button",{onClick:function(){setBtR(backtest(data,sigs));},style:{padding:"9px 22px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#00E5A0,#00B4D8)",color:"#000",fontWeight:700,cursor:"pointer",fontFamily:"inherit",fontSize:13}},"▶ 실행")
          ),
        ...
        tab==="ai"&&React.createElement("div",{style:card},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10}},
            React.createElement("div",null,React.createElement("div",{style:{fontSize:15,fontWeight:600}},"🤖 Claude AI 기술적 분석"),React.createElement("div",{style:{fontSize:11,color:C.m,marginTop:4}},"RSI · 이동평균 · 볼린저밴드 종합")),
          ...
          aiText?React.createElement("div",{style:{background:"rgba(167,139,250,0.05)",border:"1px solid rgba(167,139,250,0.15)",borderRadius:10,padding:18,lineHeight:1.8,fontSize:13}},
            aiText.split("\n").map(function(line,i){
              if(line.startsWith("**")&&line.endsWith("**"))return React.createElement("div",{key:i,style:{fontWeight:700,color:C.p,marginTop:10}},line.replace(/\*\*/g,""));
              return React.createElement("div",{key:i,dangerouslySetInnerHTML:{__html:line.replace(/\*\*(.+?)\*\*/g,"<strong style=\"color:#FFD700\">$1</strong>")||"\u00a0"}});
            })
